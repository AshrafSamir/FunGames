...
function update()
{
    if(checkIfGameOver(snakeDimention[0].x,snakeDimention[0].y))//if snake dimention is not equal to the border of the page
    {
        var getin=getInputDirection();
        for(let i=snakeDimention.length-2;i>=0;i--)//update snake dimention every one is equal to the previous one except the first one  
        {
        ...
        if(updateFood(snakeDimention[0].x,snakeDimention[0].y))//if snake first dimention is equal to the food dimention
        {
            snakeDimention[snakeDimention.length]={...snakeDimention[snakeDimention.length-1]}
                speed+=0.25;// speed up
                Time=1000/speed;
                score++;
...
function draw()//to draw the snake
{
...
{
    if(x<=0||x>=21||y<=0||y>=21)//page limit
    {
        return false;
    }
